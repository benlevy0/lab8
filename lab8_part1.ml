...
module MakeInterval (Endpoint : ORDERED_TYPE) =
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with
      | Empty -> false
      | Interval (start, endp) -> compare x start > 0 && compare x endp < 0 ;;

    (* intersect intvl1 intvl2 -- Returns the intersection of `intvl1`
       and `intvl2` *)

    let get_points (intvl : interval) : Endpoint.t * Endpoint.t =
      match intvl with
      | Interval (start, endp) -> (start, endp) ;;

    let if_empty (intvl1 : interval) (intvl2 : interval) : bool =
      let (start1, end1) = get_points intvl1 in
      let (start2, end2) = get_points intvl2 in
      (compare start1 end2 > 0) || (compare start2 end1 > 0) ;;

    let if_greater (p1 : Endpoint.t) (p2 : Endpoint.t) : bool =
      compare p1 p2 > 0 ;;
    ...
    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      ...
      if if_empty intvl1 intvl2 then Empty
      else
        let start_result = (if if_greater start1 start2 then start1 else start2) in
        let end_result = (if if_greater end1 end2 then end2 else end1) in
        Interval (start_result, end_result)
    end ;;
...
module IntegerVal : ORDERED_TYPE =
  struct
    type t = int
    let compare (t1 : 't) (t2 : 't) : int =
      if t1 < t2 then -1 else if t1 = t2 then 0 else 1
  end ;;

module IntInterval = MakeInterval IntegerVal ;;
